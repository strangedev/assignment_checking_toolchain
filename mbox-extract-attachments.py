...
__author__ = "Pablo Castellano <pablo@anche.no>, Noah Hummel <n.p.hummel@gmail.com>"
...
def extract_attachment(payload, directory):
	...
	#		filename = "unknown_%d_%d.txt" %(i, p)
		...
		try:
			fp = open(filename, "w")
			fp.write(content)
			cor = open("korrektur.txt", "w+")
			cor.write(korrektur_template)
		except IOError:
		...
		if payload.is_multipart():
			for payl in payload.get_payload():
				extract_attachment(payl, directory)
...
if len(sys.argv) == 3:
    ...
    print "directory doens't exist ", directory
      	sys.exit(1)

try:
	f = open(os.path.join(directory, "korrektur_template.txt"))
	korrektur_template = f.read()
...
for i in range(len(mb)):
	...
	if em.is_multipart():
		for payl in em.get_payload():
			extract_attachment(payl, user_dir)
	else:
		extract_attachment(em, user_dir)
